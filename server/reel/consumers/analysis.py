...
class AnalysisConsumer(WebsocketConsumer):
    ...
    def connect(self):
        ...
        """
        # TODO figure out create-on-connect properly
        self.analysis_id = str(self.scope["url_route"]["kwargs"].get("analysis_id", uuid4()))

        # Set the group name
        self.analysis_group_name = f"analysis-{self.analysis_id}"

        # Add this channel to the analysis group
        async_to_sync(self.channel_layer.group_add)(self.analysis_group_name, self.channel_name)

        # Accept the connection
        self.accept()

        # Flush the cache of messages from this analysis so far
        for message in cache.get(self.analysis_group_name, default=list()):
            self.send(text_data=message)

    def disconnect(self, close_code):
        """ Accept a disconnection from the analysis gracefully
        """
        async_to_sync(self.channel_layer.group_discard)(self.analysis_group_name, self.channel_name)
    ...
    def receive(self, text_data=None, bytes_data=None):
        ...
        try:
            ...
            if message.action == "ask":
                ask(self.analysis_id, message).send(self)
